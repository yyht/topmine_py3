...
def get_mined_word(data_path):
	with open(data_path, "r") as frobj:
		lines = frobj.read().splitlines()
		
	word_dict = {}
	for index in range(1, len(lines)):
		content = lines[index].split("&&&&")
		word = content[0]
		label = content[1]
		word_dict[word.lower()] = label
	return word_dict

def get_kfc_word(data_path):
	with open(data_path, "r") as frobj:
		lines = frobj.read().splitlines()
		
	word_dict = {}
	for index in range(1, len(lines)):
		word_dict[lines[index]] = "色情"
	return word_dict

def get_evaluation_data(data_path, debug):
	with open(data_path, "r") as frobj:
		lines = frobj.read()
	...
	for index in range(1, end_index-2):
		content = contents[index].split("\n")[1]
		try:
			label = contents[index+1].split("\n")[0]
			label = re.sub("&", "", label)
		except:
			print(index, len(contents))
		text = re.sub(pattern, "", content.lower())
			
		if text in examples:
			if label not in examples[text]:
				examples[text].append(label)
		else:
			examples[text] = [label]

	clean_data = [{"content":key.lower(), "label":examples[key][0]} 
				for key in examples if len(examples[key]) == 1]
	return clean_data
...
def kfc_word_postprocess(kfc_word_dict):
	raw_word = []
	for key in kfc_word_dict:
		word_lst = re.split("[|]", key)
		raw_word.extend(word_lst)
	raw_word = list(set(raw_word))
	only_string = [item for item in raw_word if "~" not in item]
	string_dict = {}
	for word in only_string:
		string_dict[word.lower()] = "色情"
	return string_dict

mined_word_dict = get_mined_word("/data/xuht/topmine/mined_porn_politics.txt")
print("==size of mined dict==", len(mined_word_dict))
kfc_word_dict = get_kfc_word("/data/xuht/topmine/green_kfc_porn.txt")
print("==size of kfc dict==", len(kfc_word_dict))
clean_data = get_evaluation_data("/data/xuht/topmine/test_content.txt", False)
print("==size of evaluation text==", len(clean_data))
raw_word = kfc_word_postprocess(kfc_word_dict)
print("==size of raw kfc word==", len(raw_word))

import jieba

def cut_word(word_string):
	word_lst = list(jieba.cut(word_string))
...
def and_combination(string_lst):
	return list(itertools.permutations(string_lst, len(string_lst)))

def build_trie_tree(word_dict_lst, word_type, match_type, keyword_detector):
	word_mapping = []
	for word_dict in word_dict_lst:
		for word in word_dict:
			if word_dict[word] == word_type:
				if match_type == "string":
					word_string = re.sub("&", "", word)
					keyword_detector.add_keyword(cut_word(word_string), [word_string])
					word_mapping.append(word_string)
				elif match_type == "&":
					word_string = re.split("[&]", word)
					word_str_combination = and_combination(word_string)
					word_mapping.extend(word_str_combination)
					for word_str in word_string:
						keyword_detector.add_keyword(cut_word(word_str), [word_str])
	return word_mapping
...
def detect_word(examples, output_file, keyword_detector):
	output = []
	for item in examples:
		label = item["label"]
		content = item["content"]
		detected_key_word = keyword_detector.extract_keywords(cut_word(content),
										span_info=True)
		output.append({"detected_key_word":detected_key_word, "label":label, "content":content})
	pkl.dump(output, open(output_file, "wb"))
	return output


def multi_process(examples, process_num, 
				output_folder, keyword_detector):
	...
	for chunk_id, each_chunk in enumerate(chunks):
		output_file = os.path.join(output_folder, "chunk_{}.pkl".format(chunk_id))
		pool.apply_async(detect_word,
			args=(each_chunk, output_file, keyword_detector)) # apply_async
	pool.close()
	pool.join()

def process(with_kfc):
	if with_kfc == "all":
		keyword_detector = KeywordProcessor()
		word_mapping = build_trie_tree([raw_word, mined_word_dict], "色情", "string", keyword_detector)
		print("==size of word mapping==", len(word_mapping))
		pkl.dump({"word_mapping":word_mapping, "trie_tree":keyword_detector.keyword_trie_dict},
				open("/data/xuht/topmine/result_all/string_word_all.pkl", "wb"))
		multi_process(clean_data, 20, "/data/xuht/topmine/result_all/result", keyword_detector)
	elif with_kfc == "only_mined":
		keyword_detector = KeywordProcessor()
		word_mapping = build_trie_tree([mined_word_dict], "色情", "string", keyword_detector)
		print("==size of word mapping==", len(word_mapping))
		pkl.dump({"word_mapping":word_mapping, "trie_tree":keyword_detector.keyword_trie_dict},
				open("/data/xuht/topmine/result_only_mined/string_word_only_mined.pkl", "wb"))
		multi_process(clean_data, 20, "/data/xuht/topmine/result_only_mined/result", keyword_detector)
	elif with_kfc == "only_kfc":
		keyword_detector = KeywordProcessor()
		word_mapping = build_trie_tree([raw_word], "色情", "string", keyword_detector)
		print("==size of word mapping==", len(word_mapping))
		pkl.dump({"word_mapping":word_mapping, "trie_tree":keyword_detector.keyword_trie_dict},
				open("/data/xuht/topmine/result_only_kfc/string_word_only_kfc.pkl", "wb"))
		multi_process(clean_data, 20, "/data/xuht/topmine/result_only_kfc/result", keyword_detector)

process(with_kfc="all")
process(with_kfc="only_mined")
process(with_kfc="only_kfc")

def get_result(with_kfc):
	import	os, sys

	if with_kfc == "all":
		folder = "/data/xuht/topmine/result_all/result"
		word_folder = "/data/xuht/topmine/result_all"
	elif with_kfc == "only_mined":
		folder = "/data/xuht/topmine/result_only_mined/result"
		word_folder = "/data/xuht/topmine/result_only_mined"
	elif with_kfc == "only_kfc":
		folder = "/data/xuht/topmine/result_only_kfc/result"
		word_folder = "/data/xuht/topmine/result_only_kfc"
	...
	for i in f_list:
		if os.path.splitext(i)[1] == '.pkl':
			data = pkl.load(open(os.path.join(folder, i), "rb"))
			# print([[item["label"], item["detected_key_word"]] for item in data[0:10]])
			output.extend(data)
	...
	for item in output:
		detected_word = item["detected_key_word"]
		ori_label = item["label"]

		if ori_label == "色情":
			total += 1

		if len(detected_word) >= 1:
			recall += 1
			if ori_label == "色情":
				precision += 1

		for tmp_detected in detected_word:
			if isinstance(tmp_detected[0], list):
				word = tmp_detected[0][0]
			else:
				word = item[0]
			if word in keyword:
				keyword[word]["label"].append(ori_label)
			else:
				keyword[word] = {}
				keyword[word]["label"] = [ori_label]

	pkl.dump(keyword, open(os.path.join(word_folder, "keyword.pkl"), "wb"))
	print("==total==", total, "==precision==", precision, "==recall==", recall, "==with kfc==", with_kfc)
	return keyword

keyword_all = get_result(with_kfc="all")
keyword_only_mined = get_result(with_kfc="only_mined")
keyword_only_mined = get_result(with_kfc="only_kfc")
